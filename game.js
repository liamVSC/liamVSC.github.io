const COLORS = ["red", "blue", "green", "yellow", "pink"];
    ...
    const game = {
      ...
      async loadLeaderboard() {
        ...
        try {
          if (!supabaseClient) {
            list.innerHTML = `<div class="leaderboard-empty">Leaderboard configuration is missing.</div>`;
            return;
          }

          const { data, error } = await supabaseClient
            .from("leaderboard")
            .select("player_id,name,total_stars,max_level")
          ...
          list.innerHTML = `<div class="leaderboard-empty">Couldn't load the leaderboard. Check your Supabase table and RLS policies.</div>`;
        }
      },
      ...
      renderLeaderboard(entries) {
        ...
        entries.forEach((entry, index) => {
          ...
          row.innerHTML = `
            ...
            <div class="leaderboard-name">${this.escapeHtml(entry.name || "Player")}<small>Level ${entry.max_level || 1}</small></div>
            <div class="leaderboard-score">${entry.total_stars || 0} *</div>
          `;
